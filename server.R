...
shinyServer(function(input, output) {
    ...
    predictNextEditions <- function(data, xMin, xMax) {
        ...
        predictedGoals <- function(data, team) {
            data %>% filter(Winner == team) %>% select(Goals) %>% sapply(median)
        }
        ...
        if (input$totalGoals == "Per Tournament") {
            ...
            team1 <- predictedTeam(prediction[1,])
            team2 <- predictedTeam(prediction[2,])
    ...
    perTournamentData <- function(data) {
        dt <- data %>% select(Year, Goals.for, Penalty.goal) %>% mutate(Goals = Penalty.goal + Goals.for)
    ...
    perWinnerData <- function(data, winners) {
        plotData <- merge(data, winners, by="Year") %>% select(Team, Year, Winner, Goals.for, Penalty.goal) %>%
            mutate(Goals = Goals.for + Penalty.goal) %>% filter(Team == Winner) %>% select(Year, Winner, Goals)
        list(data=plotData, nudge_y=.5, colour="darkblue")
    }
    ...
    output$linePlot <- renderPlot({
        ...
        plotData <- if (input$totalGoals == "Per Tournament") perTournamentData(data)
                    else perWinnerData(data, winnersData)
        dateFilter <- (plotData[["data"]]$Year >= xMin) & (plotData[["data"]]$Year <= xMax)
        ...
        g <- ggplot(plotData[["predicted"]], aes(x=Year, y=Goals))
        ...
        if (input$totalGoals == "Per Winner") {
            g <- g + geom_text(aes(label=Winner, fontface="bold", angle=90), colour = "white", nudge_y = -1.5)
        }
        g
    })
